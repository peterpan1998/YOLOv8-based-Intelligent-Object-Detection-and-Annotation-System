...
#     def __init__(self,origin_anno_dir, origin_image_dir, image_dir, anno_dir, verbose):
#
#         super(Mythread1, self).__init__()
#         self.origin_anno_dir = origin_anno_dir
...
#
#         cc = Coco2voc1()
#         cc.get_CK5(self.origin_anno_dir,self.origin_image_dir,self.image_dir,self.anno_dir,self.verbose)
#
#
# class Coco2voc1(QDialog):
...
#         filename, filetype = QtWidgets.QFileDialog.getOpenFileName(self, "选取json文件",
#                                                                    "D:/document/v8test/coco2voc/2017valvoc/",
#                                                                    " (*.* *.*)")
...
#             self.mythread = Mythread1(origin_anno_dir, origin_image_dir, image_dir, anno_dir, verbose)
#             self.mythread.start()
...
class Coco2voc(QDialog):
    ...
    def btn4_clicked(self):
        if self.edit1.text() == '' or self.edit2.text() == '' or self.edit3.text() == '':
            ...
            origin_anno_dir = self.coco_json_path
            verbose = True  # 是否需要看下标记是否正确的开关标记，若是true,就会把标记展示到图片上
            # self.mythread = Mythread(origin_anno_dir, origin_image_dir, image_dir, anno_dir, verbose)
            # self.mythread.start()
            self.get_CK5(origin_anno_dir, origin_image_dir, image_dir, anno_dir, verbose)
